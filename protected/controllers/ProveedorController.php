...
class ProveedorController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('view','create','update','admin','delete', 'contactos','getRazonSocial','createAjax'),
				'users'=>array('@'),
			),
	...
	public function actionContactos(){
		if(isset($_POST) && $_POST['proveedor'] != ""){
			...
			$lista_contactos = CHtml::listData(
															 	ContactoProveedor::model()->findAllByAttributes(array('nit' => $_POST['proveedor']), array('select'=>" id, nombre || ' ' || apellido as nombre_completo ")),
															 	"id",
															 	"nombre_completo"
															 );
	...
	public function actionCreateAjax()
	{
		...
		if(Yii::app()->request->isAjaxRequest){
			if(isset($_POST['Proveedor']))
			{
				$model->attributes=$_POST['Proveedor'];
				if($model->save()){
	                echo CJSON::encode(array('status'=>'success', 'content' => $this->renderPartial('_form_ajax', array('model' => $model), true)));
				}else{
					echo CJSON::encode(array('status'=>'error', 'content' => $this->renderPartial('_form_ajax', array('model' => $model), true)));
				}
			}else{
				echo CJSON::encode(array('status'=>'success', 'content' => $this->renderPartial('_form_ajax', array('model' => $model), true)));
			}
        ...
        public function actionGetRazonSocial() {
            if(Yii::app()->request->isAjaxRequest)
            {
                $nit = $_POST['nit'];
                $razon = Proveedor::model()->findByPk($nit);
                if($razon){
                    echo CJSON::encode(array('status'=>'ok', 'content'=>$razon->razon_social));
                }
                else
                {
	...
	public function actionTraerContactos(){
		if(isset($_POST) && $_POST['proveedor'] != ""){

			$lista_contactos = CHtml::listData(ContactoProveedor::model()->findAllByAttributes(array('nit' => $_POST['proveedor'])),"id","nombre");

			try{
				$combo_contactos = CHtml::tag('option', array('value'=>''), '-- Seleccione Municipio --', true);

		        foreach ($lista_contactos as $valor=>$contacto) {
		            $combo_contactos .= CHtml::tag('option', array('value'=>$valor), CHtml::encode($contacto), true);
		        }
