    ...
    jQuery.ajax({'url':url,'data':$('#genericModal div.modal-body form').serialize(),'type':'post','dataType':'json', 'success':function(data)
            {
                if (data.status == 'failure')
                {
                    ...
                    $('#genericModal div.modal-body form').submit(function(e){ e.preventDefault(); nuevaCotizacion(url); });
                }
                else
                {
...
$(document).ready(function(){
	...
	$(".ver-otras, .subir-archivos, .upload-cot, .enviar-cotizacion-a-usuario").live("click", function(e){
		e.preventDefault();
