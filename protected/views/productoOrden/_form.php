			...
			<div class="well">
	            ...
	            <div style="overflow:scroll;">
					<?php $this->widget('bootstrap.widgets.BootGridView',array(
						...
						'dataProvider'=>$reemplazos->search($orden->id),
...
<?php if($orden_actual->paso_wf == 'swOrden/aprobar_por_comite' or $orden_actual->paso_wf == 'swOrden/aprobado_por_comite' or $orden_actual->paso_wf == 'swOrden/aprobar_por_presidencia' or $orden_actual->paso_wf == 'swOrden/aprobado_por_presidencia') { ?>
	<div class="well">		
		...
		if($orden_actual->paso_wf == 'swOrden/aprobar_por_comite') {
			$this->widget('bootstrap.widgets.BootButton', array(
				'type'=>'inverse',
				'label'=>'Cargar Asistentes Habituales',
				'htmlOptions' => array(
					'onclick'=> 'jQuery.ajax({\'url\':\'/index.php/orden/cargarAsistentesHabitualesComite/id/'.$orden->id.'/comite/Compras\',\'dataType\':\'json\',\'type\':\'post\',\'success\':function(data){
...
<?php 
if(in_array($orden_actual->paso_wf,array('swOrden/gerente_compra',
		                               'swOrden/aprobar_por_comite',
									   'swOrden/aprobar_por_presidencia',
									   'swOrden/aprobar_por_junta',
									   'swOrden/aprobar_por_atribuciones'))
)
{ 
?>
	<div class="alert alert-block alert-warning fade in">		
		<?php 
		if($orden_actual->paso_wf == 'swOrden/gerente_compra'){ 

			$atribucion = Atribucion::model()->findByAttributes(array('id_empleado' => Yii::app()->user->id_empleado));
			echo  "<p>Atribuciónes Utilizadas: ".Orden::formatoMoneda($atribucion->atribucion_disponible)."</p>";
		}
		?> 
		...
		if(in_array($orden_actual->paso_wf,array('swOrden/gerente_compra',
			...
			echo "<p>Costo total de la orden: ".Orden::formatoMoneda($orden->costo_total())."</p>";
	    }
		?>
	</div>
<?php } ?>
