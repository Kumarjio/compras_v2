	...
	public function behaviors()
  {
	...
	public function rules()
	{
		...
		return array(
			...
			array('tipo_compra, negociacion_directa, nombre_compra, resumen_breve','required','on'=>"paso_1"),
			array('fake','validarNuevosProductos', 'on'=>'paso_3'),
			array('fake','validarInclusionProductos', 'on'=>'paso_3_analista'),
			//Usuario a jefe  , 'paso_1'
			array('tipo_compra, negociacion_directa, nombre_compra, resumen_breve, id_jefatura, id_jefe','required','on'=>array('sw:llenaroc_jefe','sw:jefe_gerente', 'sw:devolucion_jefe')),
			array('tipo_compra, negociacion_directa, nombre_compra, resumen_breve, id_jefatura, id_jefe','required','on'=>array('paso_1')),
			...
			array('id_vicepresidente, id_gerente','validarRequerido','on'=>array('sw:llenaroc_jefe','sw:jefe_gerente','sw:jefe_gerente','sw:gerente_analista_compras','sw:llenaroc_gerente','paso_1')),
			array('id_gerencia, id_vicepresidencia', 'validarAlguno','on'=>array('sw:llenaroc_analista_compras', 'paso_1')),
			//Gerente a compras
			...
			array('observacion','required', 'on'=>array('sw:vicepresidente_compra_devolucion')),
			array('observacion','required', 'on'=>array('sw:aprobar_por_atribuciones_devolucion')),
			...
			array('observacion', 'existe_elegido_usuario', 'on' => 'eleccion_cotizaciones'),//'on' => array('sw:validacion_cotizaciones_gerente_compra')),
			array('observacion', 'aprobar_marco', 'on' => 'eleccion_cotizaciones'),
			
			//en_negociacion a validacion_cotizaciones
			array('fake', 'existe_elegido_compras', 'on' => 'ingresar_cotizaciones'),//'on' => array('sw:en_negociacion_validacion_cotizaciones')),
            ...
            array('fake','tieneCotizaciones',  'on' => 'ingresar_cotizaciones'),//'on'=>array('sw:en_negociacion_gerente_compra')), 		
	
			//aprobar_por_comite aprobado_por_comite
			array('observacion', 'existe_elegido_comite', 'on' => 'eleccion_final_cotizaciones'),//'on' => array('sw:aprobar_por_comite_aprobado_por_comite')),
			...
			array('tipo_compra, nombre_compra, resumen_breve, id_gerencia, id_jefatura, id_gerente, id_jefe, validacion_usuario, validacion_jefe, validacion_gerente, cuenta_contable,valor_presupuestado, observacion, nombre_usuario_search,negociacion_directa,marco, id_vicepresidencia, id_vicepresidente', 'safe'),
	...
	public function relations()
	{
		...
		return array(
			...
			'ordenSolicitudes' => array(self::HAS_MANY, 'OrdenSolicitud', 'id_orden'),
	...
	public function attributeLabels()
	{
		return array(
            ...
            'id_vicepresidente' => 'Nombre Vicepresidente'
		);
	}
	...
	public function validarNuevosProductos(){
		foreach ($this->ordenSolicitudes as  $os) {
			if(!$os->save()){
				$nombre = ($os->nombre != '')? $os->nombre : 'Sin nombre';
				$this->addError("fake", "El producto $nombre tiene datos sin diligenciar aun.");
			}
		}
	}

	public function validarInclusionProductos(){
		foreach ($this->ordenSolicitudes as  $os) {
			$op = OrdenProducto::model()->findAllByAttributes(array('id_orden_solicitud'=>$os->id));
			if(!$op){
				$this->addError("fake", "Debe elegir un producto ya creado para cada solicitud (clic en el lapiz)");
				break;
			}
		}
	}
	...
	public function tieneCotizaciones(){
		$existe = OrdenProducto::model()->findAllByAttributes(array('id_orden' => $this->id, 'rechazado' => false, 'id_marco_detalle' => null));

		$prod = count($existe);

		$coti = CotizacionOp::model()->findAll(
			array(
				'select' => 'count(*),orden_producto',
				'condition' => 'orden_producto in (select id from orden_producto where id_orden = :o and rechazado = false)',
				'params' => array(':o' => $this->id),
				'group' => 'orden_producto'
			)
		);
		...
		if($prod != $cuantas_cot)
			$this->addError("fake", "Debe agregar al menos una cotización para cada producto");
	}
	...
	public function validarRequerido($atrtibute, $params){
		...
		if($this->id_gerencia != ""){
			if($this->id_gerente =="")
				$this->addError("id_gerente", "Gerente no puede ser nulo ".$this->id_gerencia);
		}
	}
	...
	public static function elecciones($compras, $comite, $usuario, $cotizacion){
		...
		if($compras == 1){
			...
			$resp = $resp."<h5><div id=\"notification-target\"></div><span class=\"label label-warning eleccion\">".OrdenProducto::model()->get_descripcion($texto_eleccion, $cotizacion->id, "Compras")."</span></h5>";
		}
		if($comite == 1){
			if($resp == ''){
				$resp = $resp."<h5><div id=\"notification-target\"></div><span class=\"label label-success eleccion\">".OrdenProducto::model()->get_descripcion($cotizacion->razon_eleccion_comite, $cotizacion->id, "Final")."</span></h5>";
			}else{
				$resp = $resp."<br/><h5><div id=\"notification-target\"></div><span class=\"label label-success eleccion\">".OrdenProducto::model()->get_descripcion($cotizacion->razon_eleccion_comite, $cotizacion->id, "Final")."</span></h5>";
			}
		}
		if($usuario == 1){
			if($resp == ''){
				$resp = $resp."<h5><div id=\"notification-target\"></div><span class=\"label label-danger eleccion\">".OrdenProducto::model()->get_descripcion($cotizacion->razon_eleccion_usuario, $cotizacion->id, "Usuario")."</span></h5>";
			}else{
				$resp = $resp."<br/><h5><div id=\"notification-target\"></div><span class=\"label label-danger eleccion\">".OrdenProducto::model()->get_descripcion($cotizacion->razon_eleccion_usuario, $cotizacion->id, "Usuario")."</span></h5>";
			}
		}
		/*if($cotizacion->productoOrden->orden0->paso_wf == 'swOrden/aprobado_por_atribuciones'){
			if($resp == ''){
...
//print_r($res);die;
    ...
    public function sendPrueba(){
    	$html = "<h2>Correo de prueba</h2>";
    	echo $html;
    	//Yii::app()->mailer->correoPrueba($html);
    }
	...
	protected function afterSave(){
      ...
      $this->enviarEmail();
		if($this->paso_wf == "swOrden/aprobar_por_comite")
			$this->enviarEmailComite();
	...
	public function enviarEmailComite(){
        $asistentes = AsistenteComiteCompras::model()->findAll();
        foreach ($asistentes as  $a) {
	        $proximo = $this->proximoPaso($paso);
			$orden = $this->findByPk($this->id);
			$empleado = Empleados::model()->findByPk($a->id_empleado);
			$email = $empleado->email;

			$urlFin = "http://".$_SERVER['HTTP_HOST']."/index.php/orden/print/orden/".$this->id;

			if($email != ""){
			  	$html = Yii::app()->controller->renderPartial("/orden/emailview", array('orden'=>$orden),true);
			  	//Yii::app()->mailer->compraComite($email, $orden->nombre_compra, $proximo, $id, $urlFin, $html);        
			}
...
EOD;
	...
	public function calcularAhorro(){
			...
			foreach ($productos_orden as $producto) {
				...
				$ahorro = 0;
				foreach ($producto->CotizacionsOp as $cot) {
...
EOD;
	...
	public function existe_elegido_usuario(){
		$productos_orden = OrdenProducto::model()->findAllByAttributes(array('id_orden' => $this->id, 'rechazado' => false, 'id_marco_detalle' => null));
		if(count($productos_orden) > 0){
			foreach($productos_orden as $po){
				$cotizaciones = CotizacionOp::model()->findAllByAttributes(array('orden_producto' => $po->id, 'elegido_usuario' => 1));
				if(count($cotizaciones) == 0){
					$this->addError("observacion", "Debe seleccionar una cotizacion para el producto ".$po->idProducto->nombre.".");
				}
			}
		}
	}

	public function aprobar_marco(){
		$productos_orden = OrdenProducto::model()->findAllByAttributes(array('id_orden' => $this->id, 'rechazado' => false), 'id_marco_detalle is not null');
		if(count($productos_orden) > 0){
			foreach($productos_orden as $po){
				if($po->aprobado_consumo == null && $po->rechazado == false){
					$this->addError("observacion", "Debe aprobar o rechazar el consumo marco del producto ".$po->idProducto->nombre.".");
				}
			}
		}
	}
	...
	public function existe_elegido_compras(){
		...
		if(count($productos_orden) > 0){
			foreach($productos_orden as $po){
				$cotizaciones = CotizacionOp::model()->findAllByAttributes(array('orden_producto' => $po->id, 'elegido_compras' => 1));
				if(count($cotizaciones) == 0){
					$this->addError("fake", "Debe seleccionar una cotizacion para el producto ".$po->idProducto->nombre.".");
				}
			}
		}
	}
	...
	public function existe_elegido_comite(){
		...
		if(count($productos_orden) > 0){
			foreach($productos_orden as $po){
				$cotizaciones = CotizacionOp::model()->findAllByAttributes(array('orden_producto' => $po->id, 'elegido_comite' => 1));
				if(count($cotizaciones) == 0){
					$this->addError("observacion", "Debe seleccionar una cotizacion para el producto ".$po->idProducto->nombre.".");
				}
			}
	...
	public function proveedores(){
		$a = array();
		$a = $this->dbConnection->createCommand("select cot.nit from cotizacion_op cot inner join orden_producto po on (po.id = cot.orden_producto) where po.id_orden=".$this->id." and cot.elegido_comite=1 group by cot.nit union select cot.nit from om_cotizacion cot inner join orden_producto po on (po.id_marco_detalle = cot.producto_detalle_om) where po.id_orden=".$this->id." and cot.elegido_comite=1 group by cot.nit;")->queryAll();
		$r = array();
		if(count($a)>0){
			foreach($a as $id){
				$r[] = $id['nit'];
	...
	public function elegirCotizacionFinal(){
		if($this->paso_wf == "swOrden/gerente_compra"){
			if($this->id_vicepresidencia == "")
				return true;
			else
				return false;
		...
		else if ($this->paso_wf == "swOrden/vicepresidente_compra")
			return true;
		return false;
	}
